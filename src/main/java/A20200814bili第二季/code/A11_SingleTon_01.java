 ...
 * @Description : 单线程环境下
 */
public class A11_SingleTon_01 {

    private volatile static A11_SingleTon_01 instance = null;

    private A11_SingleTon_01() {
        System.err.println(Thread.currentThread().getName() + "   每次创建对象时调用。。。");
    }

    public static A11_SingleTon_01 getInstance() {
        if (instance == null) {
            synchronized (A11_SingleTon_01.class) {
                if (instance == null) {
                    instance = new A11_SingleTon_01();
                }
            }
        }
        return instance;
...
//                System.err.println(Thread.currentThread().getName()+"|||"+A10_SingleTon_01.getInstance());
    ...
    private static void getOneThread() {
        System.err.println(A11_SingleTon_01.getInstance() == A11_SingleTon_01.getInstance());
        System.err.println(A11_SingleTon_01.getInstance() == A11_SingleTon_01.getInstance());
        System.err.println(A11_SingleTon_01.getInstance() == A11_SingleTon_01.getInstance());
    }
}
